...
The project includes a PowerShell based feeder script which allows you to load events from multiple ICS calendar files (or URLs, in example google calendars), find the next upcoming event and send the processed data to your MQTT broker where the Rememberall will pick it up.
...
Note that it requires 2 external libraries for MQTT communication and ICS calendar handling:
